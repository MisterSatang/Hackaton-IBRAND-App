...
export function Packaging_fail({ className }) {
    ...
    useEffect(() => {
        async function getTransaction() {
            const tran = await axios.get(
                `http://localhost:8000/transaction/find_tranid/${tran_id}`,
                {
                    headers: {
                        token: token,
                    },
    ...
    const clickSend = () => {
        axios.put(
            `http://localhost:8000/transaction/update/${transaction._id}?update=status_user`,
            {
                value: "wating",
            }
        );
        axios.put(
            `http://localhost:8000/transaction/update/${transaction._id}?update=status`,
            {
                value: "confirm",
            }
        );
        axios
            .put(
                `http://localhost:8000/transaction/update/${transaction._id}?update=file_pakaging`,
                {
                    value: link,
                }
            )
                        ...
                        <div className="col-10">
                            ...
                            <div className="mt-5">
                                ...
                                <div className="input-group mb-3 mt-3">
                                    ...
                                    <input
                                        ...
                                        placeholder="Your Link Design Product"
                            ...
                            <div className="d-flex justify-content-between mt-4">
                                ...
                                <div className="d-flex mt-4">
                                    <Link
                                        to={`/wait/wating/${transaction.fac_id}/${transaction.step}`}
                                    >
                                        <button
                                            type="button"
